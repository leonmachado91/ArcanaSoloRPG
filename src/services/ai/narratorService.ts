    ...
    for (const key in tags) {
        ...
        if (!tags[key as keyof typeof tags]) {
            ...
            throw createAppError(
                ...
                `Prompt de tag de contexto '${key}' não encontrado.`,
...
export const initializeChatSession = async (gameState: GameState): Promise<void> => {
    ...
    if (!baseSystemInstruction || !sessionStartTag) {
        ...
        throw createAppError('UNKNOWN_ERROR', 'Prompts essenciais da IA Mestre nÃ£o foram encontrados. Verifique a base de dados.', null, context);
...
export const updateAIContext = async (changeDescription: string): Promise<string> => {
    ...
    if (!stateChangeTag) {
        ...
        console.error(`[${context}] Prompt 'STATE_CHANGE_CONTEXT_TAG' nÃ£o encontrado.`);
